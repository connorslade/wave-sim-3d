...
fn main(@builtin(global_invocation_id) pos: vec3u) {
    let next = ctx.step % 3;
    let curr = (ctx.step + 2) % 3;
    let prev = (ctx.step + 1) % 3;
    ...
    // var u = ds * ctx.v - map[index(pos, prev)] + 2.0 * map[index(pos, curr)];
    ...
    // if pos.x == 50 && pos.y == 50 && pos.z == 50 {
    //     map[index(pos, next)] = 1.0;
    //     map[index(pos, curr)] = 1.0;
    //     map[index(pos, prev)] = 1.0;
    // }
